...
*/
class rate_user_positive extends \phpbb\notification\type\base
{
	/**
...
/**
	...
	*/
	...
	{
		return 'pico.reputation.notification.type.rate_user_positive';
	}
	...
	* @var string
	*/
	protected $language_key = 'NOTIFICATION_RATE_USER_POSITIVE';
	...
	*/
	...
	{
		$users = [];
		$data['user_id_to'] = (!is_array($data['user_id_to'])) ? [$data['user_id_to']] : $data['user_id_to'];
	...
	*/
	...
	{
		return $this->helper->route('reputation_details_controller', ['uid' => $this->user->data['user_id']] + ($this->get_data('auc') ? ['auc' => true] : []));
	}
	...
	*/
	public function add_voting_users($user)
	{
		...
		$voting_users = $this->get_data('voting_users');
		$voting_users = ($voting_users === null) ? [] : $voting_users;
		...
		return ['notification_data' => $serialized_data];
	}
}
